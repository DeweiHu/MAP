# -*- coding: utf-8 -*-
"""
Created on Wed Aug 31 10:02:37 2022
...
class VFTransformer_v2(nn.Module):
    ...
    def __init__(self, nch_enc, nch_out):
        ...
        for i in range(len(self.nch_enc)):
            ...
            if i == 0:
                ...
                self.decoder.append(Residual_block(self.nch_dec[i]*2, self.nch_out))
            else:
                self.decoder.append(Residual_block(self.nch_dec[i]*2, self.nch_dec[i+1]))
...
class VFT_AE(nn.Module):
    def __init__(self, enh_enc, seg_enc, nch_in, nch_latent):
        super(VFT_AE, self).__init__()
        
        self.enh_enc = enh_enc
        self.seg_enc = seg_enc
        self.nch_in = nch_in
        self.nch_latent = nch_latent
        
        self.encoder = VFTransformer_v2(self.enh_enc, self.nch_latent)
        self.decoder = res_UNet(self.seg_enc, self.nch_latent, 2)
        
    def forward(self, x):
        z, _ = self.encoder(x)
        pred, _ = self.decoder(z)
        return z, pred
        

class Contrastive_Model(nn.Module):
    def __init__(self, dyn_enc, stc_enc, seg_enc, nch_in, nch_latent):
        super(Contrastive_Model, self).__init__()
        
        self.dyn_enc = dyn_enc
        self.stc_enc = stc_enc
        self.seg_enc = seg_enc
        self.nch_in = nch_in
        self.nch_latent = nch_latent
        
        self.dyn_encoder = res_UNet(self.dyn_enc, self.nch_in, self.nch_latent)
        self.stc_encoder = VFTransformer_v2(self.stc_enc, self.nch_latent)
        self.decoder = res_UNet(self.seg_enc, self.nch_latent, 2)
        
    def forward(self, x1, x2):
        z_dyn,_ = self.dyn_encoder(x1)
        z_stc,_ = self.stc_encoder(x2)
        y_dyn,_ = self.decoder(z_dyn)
        y_stc,_ = self.decoder(z_stc)
        return z_dyn, z_stc, y_dyn, y_stc
        
        
class Adversarial_Regularize(nn.Module):
    def __init__(self, nch_enc, nch_in, nch_out, n_class):
        super(Adversarial_Regularize, self).__init__()
        
        self.nch_enc = nch_enc
        self.n_class = n_class
        self.SegNet = res_UNet(nch_enc, nch_in, nch_out)
        self.RegNet = self.Regularize()
    
    def Regularize(self,):
        num_feature = int((256/(2**len(self.nch_enc)))**2)
        len_feature = int(self.nch_enc[-1])
        net = nn.Sequential(
                      nn.Linear(num_feature*len_feature, int(num_feature*len_feature/4)),
                      nn.ReLU(),
                      nn.BatchNorm1d(int(num_feature*len_feature/4)),
                      nn.Linear(int(num_feature*len_feature/4), int(num_feature*len_feature/16)),
                      nn.ReLU(),
                      nn.BatchNorm1d(int(num_feature*len_feature/16)),
                      nn.Linear(int(num_feature*len_feature/16),self.n_class),
                      nn.BatchNorm1d(self.n_class),
                      )
        return net
    
    def forward(self, x):
        y_pred, latent = self.SegNet(x)
        latent = latent.view(latent.size(0), -1)
        class_pred = F.log_softmax(self.RegNet(latent),dim=1)
