# -*- coding: utf-8 -*-
"""
Created on Thu Feb  9 04:13:02 2023
...
optimizer_inner = torch.optim.Adam(model_anchor.parameters(),lr=1e-3)
scheduler_inner = StepLR(optimizer_inner, step_size=1, gamma=0.5)
...
for epoch in range(5):
    ...
    with tqdm(total=len(values)) as pbar:
        ...
        for step, (x,y,anchor) in enumerate(train_loader):
            ...
            pbar.update(1)
            pbar.set_description('Teacher: %d. inner-loss: %.4f.' \
                                 %(epoch+1, seg_loss.item()))
    
    torch.save(model_anchor.state_dict(),check_root + "model_anchor.pt")
#    model.load_state_dict(torch.load(check_root + "model_anchor.pt"))
...
#            sim_loss = L1_loss(z, z_anchor)
            ...
            with torch.no_grad():
                ...
                pbar.update(1)
                pbar.set_description('Student: %d. seg-loss: %.4f. sim-loss: %.4f. Dice: %.4f.' \
    ...
    valid_result["aria_diabetic"] > 0.64:
        name = 'DistilSyn_tuned_{}.pt'.format(epoch)
